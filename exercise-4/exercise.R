...
mean.gain <- mean(flights$gain, na.rm = TRUE)
print(mean.gain)
...
avg.seattle <- mean(seattle$gain, na.rm = TRUE)
print(avg.seattle)
...
jfk.to.sea <- flights %>% 
  ...
  summarise(mean = mean(air_time, na.rm = TRUE), max = max(air_time, na.rm = TRUE), min = min(air_time, na.rm = TRUE))
print(jfk.to.sea)
